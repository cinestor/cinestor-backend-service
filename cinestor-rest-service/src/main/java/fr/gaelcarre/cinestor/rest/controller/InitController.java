...
import fr.gaelcarre.cinestor.model.Scene;
import fr.gaelcarre.cinestor.model.Season;
...
public class InitController {
	...
	public void init() {
		...
		p.setSeasons(new HashSet<>());

		Season s = new Season();
		s.setNumber(1);
		s.setEpisodes(new HashSet<>());

		Season s2 = new Season();
		s2.setNumber(2);
		s2.setEpisodes(new HashSet<>());

		Episode e1 = new Episode();
		e1.setNumber(1);
		e1.setScenes(new HashSet<>());

		for (int i = 1; i < 3; i++)
			e1.getScenes().add(createScene(i));
	...
	public Scene createScene(Integer i) {
		Scene s = new Scene();
		s.setNumber(i);
		s.setDescription(
		...
		c.setName("Bob Sinclair");
		c.setSheet("Un justicier mystérieux errant dans le Far West qui est un as de la gâchette");
